  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Stack(
       ...
       // third coffee
         if(_index>1)
           _CoffeeTransforms(coffee: coffeeList[_index-2],
           scale:  lerpDouble(0.3, 0, _percent)!,
           opacity: lerpDouble(0.5, 0.0, _percent)!,
           ),
          //Second coffee
          if(_index>0)
            _CoffeeTransforms(coffee: coffeeList[_index-1],
            displacement: lerpDouble(height*.1, 0, _percent)!,
            scale: lerpDouble(.6, .3, _percent)!,
            opacity: lerpDouble(.8, .5, _percent)!,
            ),
          _CoffeeTransforms(
...
class _CoffeeTransforms extends StatelessWidget {
  const _CoffeeTransforms({Key? key, required this.coffee,
  ...
  @override
  Widget build(BuildContext context) {
    return Transform.translate(offset: Offset(0,displacement),
